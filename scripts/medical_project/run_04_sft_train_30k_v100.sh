...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'EOF'
Usage: bash scripts/medical_project/run_04_sft_train_30k_v100.sh

Run the 30k medical SFT training job on V100 32GB.
...
Useful overrides for job submission:
  ...
  OUTPUT_DIR=outputs/medical_project/sft/qwen25_7b_lora_30k_v100
  NUM_TRAIN_EPOCHS=1
...
echo "Medical SFT 30k V100 profile: method=${SFT_METHOD}, fp16, flash_attn=${FLASH_ATTN}"
